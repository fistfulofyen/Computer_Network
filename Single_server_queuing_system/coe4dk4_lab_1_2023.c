...
#define RANDOM_SEED 400132290	
#define NUMBER_TO_SERVE 10e5
...
int main()
{
	...
	double total_rejected_probability = 0;
	int a = 0;
	/* Cycle through different values of Arrival_Rate */
	for (arrival_rate_val = 0.05; arrival_rate_val < 1; arrival_rate_val = arrival_rate_val + 0.05) {
		...
		for (a = 0; a < 10; a = a + 1) {
			...
			total_rejected_probability = (double)((double)total_rejected)/ ((double)total_arrived);
		...
		printf("\n %f \t %f \t %f\n", arrival_rate_val, avg_mean_delay / a,total_rejected_probability);
		avg_mean_delay = 0;
